...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for task in orm.Task.objects.all():
            print task.id
            # print task.worker.fio.encode('koi8-r')
            # print task.client.fio.encode('koi8-r')
            task.worker_new = orm.Person.objects.get(fio=task.worker.fio)
            task.client_new = orm.Person.objects.get(fio=task.client.fio)
            task.save()
